...
def train():
  ...
  layer1 = create_conv_layer(image_shaped_input, 3, 64, [5, 5], [3, 3], conv1_w_bits, conv1_w_max, conv1_b_bits,
  ...
  def feed_dict(train):
    ...
    if train or FLAGS.fake_data:
      ...
      (xs, ys), (x_test, y_test) = cifar_data.load_data()
      k = FLAGS.dropout
    else:
      # xs, ys = mnist.test.images, mnist.test.labels
      (x_train, y_train), (xs, ys) = cifar_data.load_data()
      k = 1.0
...
def main(_):
  ...
  #
  # dataset_dir = maybe_download_and_extract('/tmp/cifar_download')
  # print(dataset_dir)
